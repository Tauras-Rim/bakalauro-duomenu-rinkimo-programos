    ...
    for month, commit_hash in monthly_commits.items():
        ...
        try:
            ...
            if global_json_exists:
                os.rename(global_json_path, global_json_path + "renamed.bak")
                    ...
                    if total is not None:
                        ...
                        for proj in chunk:
                            try:
                                ...
                                root_elem = tree.getroot()
                                package_refs = root_elem.findall(".//PackageReference")
                                total_dependencies += len(package_refs)
                            except Exception as e:
                                print(f"Error counting dependencies in {proj}: {e}")
                    else:
                        print("Could not parse libyear for chunk.")

                if total_dependencies > 0:
                    average_age = total_adjusted / total_dependencies
                    print(f"Averaged: {average_age:.2f}, Total: {total_adjusted}, dependency count: {total_dependencies}")
                    results[month] = (average_age, total_dependencies)
                else:
                    print("No dependencies found.")
                    results[month] = (0.0, 0)

            finally:
                if global_json_exists and os.path.isfile(global_json_path + "renamed.bak"):
                    os.rename(global_json_path + "renamed.bak", global_json_path)
...
def write_results_to_csv(results, repo_path, start_year=2020, end_year=2025, output_file="results_bot_examine_normalized.csv"):
    all_months = [
    ...
    if dependabot_month is None:
        output_file = "results_non_bot_examine_normalized.csv"
    else:
        output_file = "results_bot_examine_normalized.csv"
...
if __name__ == "__main__":
    ...
    print("\nFinal Results:")
    for month, value_tuple in filled_results.items():
        value = value_tuple[0]
